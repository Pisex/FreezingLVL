public APLRes AskPluginLoad2(Handle hMySelf, bool bLate, char[] sError, int iErrorSize)
{
	...
	g_hForward_Hook[LR_OnExpChangedPre] = new PrivateForward(ET_Ignore, Param_Cell, Param_CellByRef);
...
void CallForward_OnExpChangedPre(int iClient, int &iExp)
{
	Call_StartForward(g_hForward_Hook[LR_OnExpChangedPre]);
	Call_PushCell(iClient);
